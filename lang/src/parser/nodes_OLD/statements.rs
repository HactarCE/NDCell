...
impl SyntaxConstruct for StatementBlock {
    ...
    fn eat(&self, tf: &mut Parser<'_>) -> LangResult<Self::Output> {
        tf.parse(PunctuationToken::LBrace)?;
        std::iter::from_fn(|| {
            feed_one_of!(
                tf,
                [
...
impl SyntaxConstruct for Statement {
        ...
        match tf.next_class() {
            Some(TokenClass::Keyword(kw)) if kw.starts_statement() => match kw {
                ...
                For => Ok(tree::Statement::ForLoop {
                    var_expr: tf.parse(Expr)?,
                    iter_expr: {
                        tf.parse(KeywordToken::In)?;
                        tf.parse(Expr)?
                    },
                    block: tf.parse(StatementBlock)?,
                }),
                ...
                // Returning values
                Become => Ok(tree::Statement::Become(tf.parse(Expr)?)),
                Remain => tf.err(Unimplemented),
                Return => Ok(tree::Statement::Return(tf.parse(Expr)?)),
                ...
                If => {
                    tf.prev();
                    tf.parse(IfStatement)
                }
                Match => tf.err(Unimplemented),
                Unless => Ok(tree::Statement::If {
                    cond_expr: tf.parse(Expr)?,
                    if_true: vec![],
                    if_false: tf.parse(StatementBlock)?,
                }),

                // Debugging
                Assert => {
                    tf.prev();
                    tf.parse(AssertStatement)
                }
                Error => Ok(tree::Statement::Error {
                    msg: tf.try_parse(StringLiteral.spanned()).transpose()?,
                }),
            ...
            _ => {
                tf.prev();
                tf.try_parse(VarAssignStatement)
...
impl SyntaxConstruct for IfStatement {
    type Output = tree::Statement;
    fn might_match(&self, tf: Parser<'_>) -> bool {
        next_token_matches!(tf, TokenClass::Keyword(KeywordToken::If))
    }
    fn eat(&self, tf: &mut Parser<'_>) -> LangResult<Self::Output> {
        tf.parse(KeywordToken::If)?;
        Ok(tree::Statement::If {
            cond_expr: tf.parse(Expr)?,
            if_true: tf.parse(StatementBlock)?,
...
#[derive(Debug, Copy, Clone)]
struct ElseBlock;
impl_display!(ElseBlock, "'else' statement");
impl SyntaxConstruct for ElseBlock {
    type Output = tree::StatementBlock;
    fn might_match(&self, tf: Parser<'_>) -> bool {
        next_token_matches!(tf, TokenClass::Keyword(KeywordToken::Else))
    }
    fn eat(&self, tf: &mut Parser<'_>) -> LangResult<Self::Output> {
        tf.parse(KeywordToken::Else)?;
...
impl SyntaxConstruct for AssertStatement {
    type Output = tree::Statement;
    fn might_match(&self, tf: Parser<'_>) -> bool {
        next_token_matches!(tf, TokenClass::Keyword(KeywordToken::Assert))
    }
    fn eat(&self, tf: &mut Parser<'_>) -> LangResult<Self::Output> {
        tf.parse(KeywordToken::Assert)?;
        Ok(tree::Statement::Assert {
            expr: tf.parse(Expr)?,
            msg: {
                if let Some(()) = tf.try_parse(PunctuationToken::Comma).transpose()? {
                    Some(tf.parse(StringLiteral.spanned())?)
                } else {
                    None
                }
            },
...
impl_display!(VarAssignStatement, "variable assignment statement");
impl SyntaxConstruct for VarAssignStatement {
    type Output = tree::Statement;
    fn might_match(&self, mut tf: Parser<'_>) -> bool {
        tf.parse(Expr).is_ok() && AssignOp.might_match(tf)
    }
    fn eat(&self, tf: &mut Parser<'_>) -> LangResult<Self::Output> {
        Ok(tree::Statement::SetVar {
            var_expr: tf.parse(Expr)?,
            assign_op: tf.parse(AssignOp)?,
            value_expr: tf.parse(Expr)?,
        })
    }
}
