...
impl TokenEater for Expr {
    ...
    fn eat(&self, tf: &mut TokenFeeder<'_>) -> LangResult<Self::Output> {
        ExprWithPrecedence::default().eat(tf)
    }
}
    ...
    fn eat(&self, tf: &mut TokenFeeder<'_>) -> LangResult<Self::Output> {
        ...
        match self.0 {
            ...
            OpPrecedence::Postfix => {
                let mut ret = tf.feed(Self(self.0.next()))?;
...
fn eat_binary_op(
    ...
    let mut ops = vec![];
    let mut exprs = vec![tf.feed(expr_eater)?];
    while let Some(op_fn) = tf.peek_next_class().and_then(&op_to_expr) {
        tf.next();
        ops.push(op_fn);
        exprs.push(tf.feed(expr_eater)?);
    }
    let mut ret;
    match assoc {
        Associativity::Left => {
...
impl_display!(ComparisonChain, "comparison");
impl TokenEater for ComparisonChain {
    type Output = Spanned<tree::Expr>;
    fn might_match(&self, tf: TokenFeeder<'_>) -> bool {
...
impl_display!(AttrGetSuffix, "method call");
impl TokenEater for AttrGetSuffix {
    ...
    fn eat(&self, tf: &mut TokenFeeder<'_>) -> LangResult<Self::Output> {
        tf.feed(PunctuationToken::Period)?;
        let attribute = tf.feed(Identifier.spanned())?;
...
struct FnCallSuffix;
impl_display!(FnCallSuffix, "method call");
impl TokenEater for FnCallSuffix {
    type Output = Box<dyn FnOnce(Box<Spanned<tree::Expr>>) -> Spanned<tree::Expr>>;
    fn might_match(&self, tf: TokenFeeder<'_>) -> bool {
        List::paren_comma_sep(Expr).might_match(tf)
    }
    fn eat(&self, tf: &mut TokenFeeder<'_>) -> LangResult<Self::Output> {
        let spanned_args = tf.feed(List::paren_comma_sep(Expr).spanned())?;
...
struct IndexSuffix;
impl_display!(IndexSuffix, "method call");
impl TokenEater for IndexSuffix {
    type Output = Box<dyn FnOnce(Box<Spanned<tree::Expr>>) -> Spanned<tree::Expr>>;
    fn might_match(&self, tf: TokenFeeder<'_>) -> bool {
        List::bracket_comma_sep(Expr).might_match(tf)
    }
    fn eat(&self, tf: &mut TokenFeeder<'_>) -> LangResult<Self::Output> {
        let spanned_args = tf.feed(List::bracket_comma_sep(Expr).spanned())?;
        let args_span = spanned_args.span;
        let args = spanned_args.inner;
