...
use super::{StatementBlock, UserFunction};
use crate::errors::*;
use crate::parser::{Directive, DirectiveContents, HelperFunc, ParseTree};
use crate::{data::LangInt, CellState};
...
impl<T, U> Lazy<T, U> {
    pub fn get_or_start(&mut self) -> Lazy<T, &mut U> {
        match self {
            Lazy::NotStarted(_) => std::mem::replace(self, Self::InProgress),
            _ => self,
        }
    }
}
...
impl RuleBuilder {
    pub fn tokens(&mut self) -> LangResult<Vec<Tokens>> {
        match self.tokens.get_or_start() {
            Lazy::NotStarted(_) => self._compute_tokens(),
            Lazy::InProgress => internal_error!("Recursion while asking for tokens"),
            Lazy::Done(tokens) => tokens,
        }
    }
}
...
pub struct RuleBuilder {
    ...
    tokens: Lazy<Vec<Token>>,

    // /// Number of dimensions.
    // pub ndim: Lazy<a, usize>,
    /// Neighborhood shape.
    pub nbhd_shape: PatternShape,

    /// Helper functions.
    pub helper_functions: HashMap<String, Lazy<(FnSignature, StatementBlock), UserFunction>>,
    ...
    states: Lazy<RuleStatesDescription, Vec<CellState>>,
    // helper_functions: ,
    transition_function: Lazy<StatementBlock, UserFunction>,
}
