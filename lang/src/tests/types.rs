...
fn test_vector_types() {
    ...
    let expected = ("y", "Type error: expected vec3 but got vec1");
    assert_compile_error(source_code, expected);
    ...
    let expected = ("[1, 2, 3, 4]", "Type error: expected vec3 but got vec4");
    assert_compile_error(source_code, expected);
    ...
    let expected = ("12", "Type error: expected vec1 but got integer");
    assert_compile_error(source_code, expected);
}
