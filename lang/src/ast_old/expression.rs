...
use ErrorKind::{
    CannotAssignToExpr, CannotEvalAsConst, CannotIndexType, Expected, InternalError,
    InvalidArguments, NoSuchFunction, NoSuchMethod, ReservedWord,
};
    ...
    match &parse_tree.inner {
        ...
        parser::Expr::GetAttr { object, attribute } => {
            ...
            func = functions::lookup_method(ty, &attribute.inner)
                .ok_or_else(|| NoSuchMethod(ty).with_span(attribute.span))?;
        }
        // Function call
        parser::Expr::FnCall {
            func: func_expr,
            ...
            match &func_expr.inner {
                parser::Expr::TypeName(type_token) => {
                    ...
                    func = functions::lookup_type_function(*type_token)
                        .ok_or_else(|| NoSuchFunction.with_span(func_expr.span))?;
                }
                ...
                parser::Expr::GetAttr {
                    ...
                    func = functions::lookup_method(ty, &method_name.inner)
                        .ok_or_else(|| NoSuchMethod(ty).with_span(method_name.span))?;
                }
...
impl FuncCallInfoMut<'_> {
    ...
    pub fn add_error_point(&mut self, error: ErrorKind) -> ErrorPointRef {
