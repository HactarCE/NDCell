...
fn test_range_new() {
    ...
    assert_fn_result(&mut f, &mut [], ok_range(0, 0, 1));
}
...
fn test_range_construct() {
    ...
    // Test with start < end.
    assert_fn_result(&mut f, &mut int_args(&[1, 10]), ok_range(1, 10, 1));
    assert_fn_result(&mut f, &mut int_args(&[-5, 10]), ok_range(-5, 10, 1));
    // Test with start > end.
    assert_fn_result(&mut f, &mut int_args(&[10, 1]), ok_range(10, 1, -1));
    assert_fn_result(&mut f, &mut int_args(&[5, -10]), ok_range(5, -10, -1));
    // Test with start == end.
    assert_fn_result(&mut f, &mut int_args(&[2, 2]), ok_range(2, 2, 1));
    assert_fn_result(&mut f, &mut int_args(&[-1, -1]), ok_range(-1, -1, 1));
    assert_fn_result(&mut f, &mut int_args(&[0, 0]), ok_range(0, 0, 1));
}
...
fn test_range_step_by() {
    ...
    assert_fn_result(&mut f, &mut int_args(&[1, 10, 2]), ok_range(1, 10, 2));
    assert_fn_result(&mut f, &mut int_args(&[-1, -10, 2]), ok_range(-1, -10, 2));
    assert_fn_result(&mut f, &mut int_args(&[1, 10, -2]), ok_range(1, 10, -2));
    assert_fn_result(&mut f, &mut int_args(&[-1, -10, -2]), ok_range(-1, -10, -2));
}
