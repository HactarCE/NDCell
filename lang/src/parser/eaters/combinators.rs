...
impl<T: TokenEater> TokenEater for TokenSpanner<T> {
...
impl<B, T: TokenEater, F: Fn(T::Output) -> B> TokenEater for TokenMapper<T, F> {
    type Output = B;
    fn might_match(&self, tf: TokenFeeder<'_>) -> bool {
        self.eater.might_match(tf)
    }
    fn eat(&self, tf: &mut TokenFeeder<'_>) -> LangResult<Self::Output> {
        self.eater.eat(tf).map(&self.f)
    }
}
...
impl<T: Copy + TokenEater> TokenEater for Surround<T> {
...
impl<T: Copy + TokenEater> TokenEater for List<T> {
...
impl TokenEater for Epsilon {
    type Output = ();
    fn might_match(&self, _tf: TokenFeeder<'_>) -> bool {
        true
    }
    fn eat(&self, _tf: &mut TokenFeeder<'_>) -> LangResult<Self::Output> {
        Ok(())
    }
}
