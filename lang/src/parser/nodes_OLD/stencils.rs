...
impl SyntaxConstruct for StencilLiteral {
    ...
    fn eat(&self, tf: &mut Parser<'_>) -> LangResult<Self::Output> {
        tf.parse(PunctuationToken::Backtick)?;
        let mut cells = vec![];
...
impl SyntaxConstruct for StencilCells {
...
impl SyntaxConstruct for QuotedStencilCell {
...
impl SyntaxConstruct for StencilBindings {
    type Output = Vec<tree::StencilBinding>;
    fn might_match(&self, tf: Parser<'_>) -> bool {
        next_token_matches!(tf, TokenClass::Keyword(KeywordToken::Where))
    }
    fn eat(&self, tf: &mut Parser<'_>) -> LangResult<Self::Output> {
        tf.parse(KeywordToken::Where)?;
        let mut ret = vec![];
        loop {
            let cell = tf.parse(QuotedStencilCell.spanned())?;
            tf.parse(KeywordToken::Is)?;
            let same = tf.try_parse(KeywordToken::Same).transpose()?.is_some();
            let expr = Box::new(tf.parse(ExprWithPrecedence(OpPrecedence::Is.next()))?);
            ret.push(tree::StencilBinding { cell, same, expr });
            if tf.try_parse(KeywordToken::And).transpose()?.is_none() {
                return Ok(ret);
            }
        }
    }
